...
```ts
  ...
  2  | 
  3  | 
  4  | test('Screenshot and Visual Comparison', async({page})=>{
  5  | 
  6  | await page.goto('https://www.rediff.com/');
  7  | expect(await page.screenshot()).toMatchSnapshot('landing.png');
  8  | })
  9  | 
  10 | test.only('Page & element Screenshot', async({page})=>{
  11 | 
  ...
  15 | await page.locator('#displayed-text').screenshot({path: 'elementScreenshot.png'});
  16 | await page.locator('#hide-textbox').click();
> 17 | await page.screenshot({path: 'pageScreenshot.png'});
     |            ^ Error: page.screenshot: Test timeout of 30000ms exceeded.
  18 | await expect(page.locator('#displayed-text')).toBeHidden();
  19 | })
```